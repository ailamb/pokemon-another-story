...
import { GameState, StorySegment, Choice, HistoryItem } from './types';
import * as chapter1 from './services/chapter1';
import * as chapter2 from './services/chapter2';
import * as chapter3 from './services/chapter3';
import * as chapter4 from './services/chapter4';
import * as chapter5 from './services/chapter5';
import * as chapter6 from './services/chapter6';
import * as chapter7 from './services/chapter7';
import * as chapter8 from './services/chapter8';
import * as chapter9 from './services/chapter9';
import * as chapter10 from './services/chapter10';
import GameInterface from './components/GameInterface';
import { BookOpen, AlertCircle, Play, Lock } from 'lucide-react';
...
export default function App() {
  ...
  // Helper to maintain conversation history for the AI/Log
  ...
  const handleChoice = useCallback(async (choice: Choice) => {
    // Special handling for restart to go back to Menu
    ...
    setIsLoading(true);
    // Add user choice to history logs for context
    addToHistory('user', `Choice: ${choice.text}`); // Log the readable text
  ...
  // Convert raw history to readable logs for the UI
  const getReadableHistory = () => {
    return history
      .filter(h => h.role === 'model') // Only show what was spoken/narrated
      .map(h => {
        try {
    ...
    return (
      <div className="h-screen w-full bg-black flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl w-full bg-gray-900/80 backdrop-blur-xl p-8 rounded-2xl shadow-2xl border border-gray-700 relative z-10 flex flex-col md:flex-row gap-8 max-h-[90vh]">
          ...
          {/* Header Section (Mobile Top / Desktop Left) */}
          ...
          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3">
             ...
             {CHAPTERS.map((chapter) => (
               <button
                 ...
                 `}
               >
                 {/* Thumbnail (Only for active or placeholder) */}
                 ...
                 <div className="flex-1">
                    <div className="flex justify-between items-center mb-1">
                        <span className={`text-xs font-bold tracking-wider ${chapter.active ? 'text-yellow-600' : 'text-gray-600'}`}>
                            {chapter.title}
                        </span>
                    ...
                    <h4 className={`font-medium font-serif text-lg ${chapter.active ? 'text-white group-hover:text-yellow-100' : 'text-gray-500'}`}>
                        {chapter.subtitle}
                    </h4>
                 </div>
               </button>
